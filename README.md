...
- **版本提取修复（用户反馈）**：有些缺陷把版本写在重现步骤里、字段却填"主干"，导致导入版本错误——现"主干/trunk/无/-"等占位值视为未填，从重现步骤提取（真实采样 100 条详情：56 条字段为"主干"；支持 软件/固件/整机/设备/应用/APP版本 及纯"版本："标签，"2.3.34/8.1.4.1"取 2.3.34，"版本：安卓"无数字跳过并保留原值；字段有实际值时仍优先）
- **去重增强**：Tier2 标题模糊匹配新增"头部子串"二次召回——实测 TB 全文检索为"词元全包含"召回（标题加/删字即搜不到），此前"TB 上没写禅道编号、标题又被改过字"的已导入缺陷会被重复导入；二次召回仅在完整标题"零有效候选"时启用，且用更高阈值（≥0.92）防"同模板仅尾部差异"误判；外部TB 源比对前锚定剥掉内部任务标题的源标签前缀（【323A-24】）
- **协同学习**：合并冲突时间戳改为解析比较（未补零月份、"T"/空格混写不再误判新旧，修复审核结论来回翻转）；"无变更需要推送"也刷新并持久化时间戳（此前 interval 到期后退化为每小时真跑一次 push）；状态文件原子写
- **审核预算**：调用/时长预算检查下沉到拆半递归内部（此前仅批次间检查，单个失败批最坏 ~19 次调用可突破 30 分钟预算）
- **关闭同步**：模块解析失败产品的已关闭缺陷保留并逐条取详情比对（与主同步策略一致；详情获取失败保守跳过，不关闭未经验证的任务）
- **钉钉机器人修复**：
  - 回调指令提交曾引用不存在的函数名（NameError），同步/试运行/列出bug 全部 500、后台任务从未执行 → 改为模块级线程池
  - "列出bug"支持多产品/多项目循环拉取；指派人筛选改读公用 assignee 配置（与同步口径一致）；长驻服务每次指令创建的客户端显式关闭
- **更新器修复**：ghfast 镜像因 URL 内嵌 raw.githubusercontent.com 被误判为"GitHub 直连"优先取版本（其 CDN 有缓存）→ 改为按主机名精确判断；`compare_versions` 混合类型不再抛 TypeError；`update.yaml.example` 的 version_file 路径修正为仓库根
- **导出工具**：已导入过滤补上 CPAX 前缀（此前只认 VLNS 会重复导出）；单条详情获取失败跳过并计数，不再中断整个导出
- **GUI**：添加指派人按白名单部门前缀清洗（普通名字里的 "-" 不再被误切）
- **其他**：query_ids 切到项目根目录再读配置（并输出真实错误）；feedback_automation 知识库补丁默认只生成、需 `--apply` 显式应用并自动备份+原子替换；教程"状态"选项更新；teambition.yaml.example 标注已废弃的 assignee_category_map
- **测试**：新增钉钉回调路由冒烟（4）、更新器版本比较/镜像判定（3）、指派人清洗（1）、去重层级顺序（3）、Tier2 召回兜底与阈值（4）、版本占位提取（5）、协同合并时间戳（2），共 438 个测试全过

### v2.8.8 (2026-09-11)

- **安全**：修复打包泄露——历史构建把 `configs/*.bak`（含未剥离的真实密钥）打进公开 Release zip；备份改放 %TEMP%，并新增"zip 内含 .bak 即中止发布"的校验闸门
- **引擎/同步**：
  - 关闭同步：TB 关闭 API 失败不再误报"成功"（不计数/不写关闭评论/不发通知）；查找任务补项目匹配+标题正则+归档三重校验（防关错任务）
  - 外部TB 多项目：uniqueId 按项目命名空间化（去重/详情不再串号）；项目编号前缀按项目解析；主同步与关闭同步改读顶层 project_id（多项目此前未接通）
  - 模块过滤：部分产品解析失败时保留其缺陷逐条比对（不再整批静默丢弃）；"模块不在产品树"与"解析失败"语义区分
  - 严重程度：字母 S 原样透传（外部TB 存在单独 S 值）；自定义字段检测不再误报
  - 客户端：禅道 401 重认证耗尽显式报错（不再静默返回空数据）；网络重试覆盖超时类异常；认证请求补超时；任务编号剥前缀修正；评论拉取加缓存
- **AI/分类**：
  - AI 日志分析：思维链耗尽翻倍 token 重试；JSON 校验警告不再拼进正文（此前导致下游解析全挂）
  - 审核链：翻倍重试不占常规预算；总预算熔断（防服务慢挂数小时）；输出无法解析按失败处理；失败样本零匹配告警
  - 知识库：配置层级修正（RAG 开启即生效）；检索按 record id 映射（不再错位）；rejected 入索引（反面教材可用）；协同合并冲突取时间戳新者
- **GUI/通知/工具**：
  - 协同学习：定时推送时间戳持久化（此前从未真正触发）；同步改后台线程（不再卡界面）；定时器重入清理
  - 定时同步：启动成功才消费去重键（不再丢当天时间点）；外部TB 状态筛选恢复（防保存时静默关掉关闭同步）；定时时间非法值不再导致启动崩溃
  - 钉钉去重：发送后记录真实结果（失败不再被当成功吞掉）
  - DRC/报告：事件链噪音过滤修正（裸 '0' 误杀告警）；HTML 报告日志转义；checkpoint 原子写+损坏保护
  - 提取器：跨年日期回退（未来差值超 180 天按上一年）；时区参考修复；时间窗口坐标修正
  - 发布脚本：全部 HTTP/git 命令补超时

### v2.8.7 (2026-09-11)

- **修复**：AI 审核/LLM 分类"模型可用但持续失败"的真正根因——**推理模型思维链占满输出 token 预算**（实测复现 + 修复）
  - 根因：deepseek-v4-pro 等推理模型把思维链计入 `max_tokens`，30 条/批审核时思维链需 6k~26k 字符（波动大），`max_tokens=4000` 被思考链耗尽 → `finish_reason=length`、content 为空；且 `max_retries=1` 使"翻倍 token 重试"逻辑拿不到第二次机会（`continue` 后循环直接结束），实为死代码
  - 修复：审核请求 max_tokens 4000→16000（实测 batch=30 连续 3 轮全部成功）；翻倍重试上限 8000→32000；单条分类默认 400→1200（400 时思维链同样被截断，LLM 分类层此前未真正生效）；`max_retries` 默认改为 2、`timeout` 默认 180s
  - 模型对比实测（真实审核 prompt）：glm-5.3-flash 思维链最长最慢（已淘汰）；deepseek-v4-flash 最快但稳定性差；**deepseek-v4-pro 最稳，定为默认审核模型**
- **修复**：AI 审核失败重试改为**定向重试**——重试标记记录失败批次的样本清单，下次同步只重审这些样本（此前是全量重抽检，失败样本可能根本不在重试范围内）；兼容旧标记文件回退全量重审
- **工具**：新增 `tools/model_bench_review.py` 模型对比脚本（真实审核 prompt 实测延迟/成功率/输出可解析率）

### v2.8.6 (2026-09-11)

- **修复**：AI 审核批失败导致审核持续不完整
  - 根因：审核按 `batch_size=30` 一批发送，大批量 prompt（30 条样本+39 分类描述）内网推理服务处理失败；`max_retries=1` 一次失败即整批跳过，重试审核（v2.8.4 机制）再次用同样大批 → 再次失败
  - 修复：批失败时**自适应拆半重试**（30→15→7→3→1），单条仍失败才计入失败批次
- **修复**：分类器准备阶段（模型训练/保存）异常不再中断整个同步（失败仅告警，分类走类型映射兜底）
- **文档**：CLAUDE.md 更正分类兜底描述（第四层为 Bug 类型映射，部门兜底已于 v2.7.0 移除）

### v2.8.5 (2026-09-09)

- **修复**：日志附件字段更新偶发失败（同批任务交替成功/失败）
  - 根因：TB 偶发返回 HTTP 200 但 body 为 `{"error":"Forbidden","message":"...context canceled"}`，`_request` 只按状态码判成功，错误被静默吞掉且无重试 → 附件已上传但未关联到"日志附件"字段
  - 修复：更新响应校验 `error` 字段，抛错走 `_retry` 指数退避重试；仍失败时明确告警

### v2.8.4 (2026-09-04)

- **修复**：全面代码审查（GUI 层 + 引擎层 22 项）
- **高危**：
  - 外部TB 多项目拉取缓存被清空 → 缓存未命中创建"任务 {id}"垃圾任务、评论去重失效、附件索引复用错乱（缓存改累积 + 文件判重）
  - 多产品/项目拉取单个组合失败终止整个同步 → 异常保护记录后继续
  - 配置对话框产品ID列表值显示污染（`[11, 20]` → 保存写回字符串 → 静默变成不限产品）
  - 外部TB 项目ID 手动编辑不回写（同步/保存时丢失，多项目功能链路断裂）
  - 禅道 product+project 同传时项目维度假过滤（产品接口静默忽略项目）→ 改项目路径 + 客户端产品过滤
- **中危**：关闭同步支持仅配项目ID；任务编号查询剥任意项目前缀（防止关闭同步误关）；0 字节下载守卫（三处上传通道不再上传空文件）；试运行跳过备注媒体收集（省 Selenium 开销）；定时同步 ±1 分钟补触发窗口；协同学习定时器重入保护；更新检查回调 busy 竞态；severity 翻译合并首产品优先；污染字符清洗补全（模块ID/项目ID）；AI 审核异常保护与标记全路径消费；字段名称瞬时失败不缓存
- **低危**：中文时间分钟补零（"10时5分"）；加载定时配置屏蔽信号防中间态写盘；手动检查更新语义修正

### v2.8.3 (2026-08-31)

- **新增**：定时同步支持一天最多**两个时间点** —— GUI 新增"第二时间"选中框（默认不启用）+ 时间选择器（默认 18:00），每个时间点当天各触发一次（按 日期:时间 去重）；配置 `time2_enabled`/`time2`，旧配置兼容
- **修复**：定时同步配置保存后重启不生效 —— dict 值被 `str()` 成 Python 字面量字符串写入 YAML（读回是字符串被丢弃）+ 旧块映射子行残留导致非法 YAML；修复为 JSON 序列化写入 + 清除残留块 + 加载兼容旧损坏值
- **新增**：AI 分类审核失败自动重试 —— 审核中 LLM 请求失败的批次此前只是跳过且无记录；现在失败时写重试标记，下次同步自动重跑审核流程（成功清标记，仍失败继续等下次）
