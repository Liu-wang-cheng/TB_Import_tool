"""一键发布脚本：打包后自动上传 zip 到 GitHub Release 并更新 version.json

用法：
    python release.py <version> <zip_path> <release_asset_name>
    python release.py 1.5 "dist\智能缺陷管理平台.zip" "SmartDefectPlatform.zip"
...
def update_version_json(repo: str, token: str, version: str,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        os.makedirs(os.path.dirname(vpath) if os.path.dirname(vpath) else tmpdir, exist_ok=True)
...
def main():
    if len(sys.argv) < 4:
        print("用法: python release.py <version> <artifact_path> <release_asset_name>")
        print('示例: python release.py 1.5 "dist\\智能缺陷管理平台.zip" "SmartDefectPlatform.zip"')
        sys.exit(1)

    version = sys.argv[1]
    artifact_path = sys.argv[2]
    release_asset_name = sys.argv[3]
    ...
    print(f"[3/4] 上传 {release_asset_name} ({os.path.getsize(artifact_path)/1048576:.0f}MB)...")
    download_url = upload_asset(repo, token, release_id, artifact_path, release_asset_name)
